# Autogenerated wrapper script for GR_jll for x86_64-apple-darwin14
...
const libGKS_splitpath = ["lib", "libGKS.dylib"]
...
const libGKS = "@rpath/libGKS.so"


# Relative path to `libGR`
const libGR_splitpath = ["lib", "libGR.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libGR = "@rpath/libGR.so"


# Relative path to `libGR3`
const libGR3_splitpath = ["lib", "libGR3.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libGR3 = "@rpath/libGR3.so"


# Relative path to `libGRM`
const libGRM_splitpath = ["lib", "libGRM.dylib"]
